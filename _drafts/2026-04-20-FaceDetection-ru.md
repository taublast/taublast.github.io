...
Когда речь идет о распознавании из потока камеры, хорошим варантом будет пакет `DrawnUi.Maui.Camera`. В [предыдущей статье](../VideoRecording/) я показывал, как использовать `SkiaCamera` для отправки аудио в AI в реальном времени, - сегодня же займемся видео кадрами: разберем на примере **распознавания лиц**.
