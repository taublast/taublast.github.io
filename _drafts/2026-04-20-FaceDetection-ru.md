...
Сегодняшние приложения для мобильных и настольных устройств умеют распознавать на изображениях почти что угодно, - от QR-кодов до количества калорий в еде на на фото. На платформах, которые поддерживает **.NET MAUI**, для этого можно использовать разные варианты, как локальные ML-движки вроде **TensorFlow Lite**, нативные SDK для конкретной платформы, как, например, **ARKit** на iOS, или удаленные vision API. Дальше уже все зависит от архитектуры вашего приложения.

Когда речь идет о распознавании из потока камеры, хорошим варантом будет пакет `DrawnUi.Maui.Camera`. В [предыдущей статье](../VideoRecording/) я уже показывал, как использовать `SkiaCamera` для отправки аудио в AI в реальном времени, - сегодня же займемся видео кадрами: разберем на примере **распознавания лиц**.

[Приложение-пример](https://github.com/taublast/DetectFaces), которое идет вместе с этой статьей, использует локальное распознавание лицевых landmark-точек через **MediaPipe Tasks**. Я выбрал этот вариант ради максимально единообразного поведения на всех платформах: на **iOS, Android и Windows**.

Заодно наше приложение рисует оверлеи и приклеивает маски к движущимся лицам.
...
О том, как установить и инициализировать `SkiaCamera`, я уже писал в [предыдущей статье](../VideoRecording/). В данном примере мы используем XAML и размещаем унаследованный элемент внутри обычного лейаута .NET MAUI.
...
Структура `RawCameraFrame` содержит `SKImage`, живущий на GPU, и сопутствующие метаданные.
Обычно нам нужно уменьшить изображение, правильно его повернуть и в некоторых случаях чуть подзумить, чтобы по сути обрезать лишние поля, которые не релевантны для распознавания. Все инструменты для этого уже готовы.

## Для локальной ML модели

Структура `RawCameraFrame` предоставляет метод `TryGetRgba(width, height, buffer, orientation, cropRatio)`, который заполняет заранее выделенный `byte[]` RGBA-пикселями в том финальном размере, который нужен вашей модели.

В приложении-примере используется `cropRatio` по умолчанию, то есть `1` без дополнительного зума, и `orientation` по умолчанию `OutputOrientation.Display` - в данном приложении нам не важно, чтобы картинка была строго "головой вверх"; нам важно получить ровно то, что пользователь видит на экране, даже если устройство повернуто в landscape.

Если для вашей модели важна ориентация "головой вверх", можно использовать `OutputOrientation.Portrait`. И, возможно, вам захочется подрезать кадр, например убрать края, если нужный объект почти наверняка находится ближе к центру. Для этого можно уменьшить `cropRatio`. Например, `0.9` означает, что вы срежете пропорцию `0.1` по краям.
