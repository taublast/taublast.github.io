---
layout: post
title: "The Quest Of Creating Drawn .NET MAUI Apps"
...
A deed I would like to remember is what Adam Pedley and other enthusiasts have done in an attempt [to make Xamarin drawn](https://github.com/adamped/xamarin.flutter).  
A lot of developers and companies have created their MAUI drawn controls ever since.  
...
One day after evaluating a new Figma design I have started a challenge to make this app totally drawn with SkiaSharp.  
...
This is still an experiment for a commercial app, but the final result is somewhat interesting:
...
- Custom camera control displays and sends frames to ф TensorFlow ML model
...
Some native views were placed seamlessly over the canvas: web pages for “Privacy” and “Terms Of Use” and a customized MAUI Entry for input. Those are wrapped inside `SkiaMauiElement` which can take a snapshot of a native view to draw and animate it over the canvas.  

A lot of interesting problems have been solved while making the app. 
For example, the page with insect details first renders a short info, then loads a full one from API and have to re-render seemingly while scrolling. 
Moreover, there is an unknown number of wiki articles defined by a  `DataTemplate` in XAML, that must then be instantiated  without making UI to freeze while constructing. Then they load optional banner images from the Web, all of this while scrolling.  

An `ImageDoubleBuffered` cache type was then born: to render the last prepared cache while constructing a new one in the background.   

Instead of the usual `ListView` or `CollectionView` we have this:
...
Prebuilt elements are `SkiaControl`, `SkiaLayout`, `SkiaShape`, `SkiaLabel`, `SkiaImage`, `SkiaSvg` and many others, easily subclassable, virtual methods, with no private API. The main idea behind is to make the toolset reusable and customizable at will. They have usual MAUI `VisualElement` properties like `BackgroundColor`, `HorizontalOptions` and so on, along with many supplementary additions.  

There are some key differences with MAUI though, one of the noticeable is that `HorizontalOptions` and `VerticalOptions` are not `Fill` by default, if you just write a `<SkiaLayout/>` it will not display, as it had `Start` layout options, an empty content and no size requests.   
Some also shipped drawn controls built on top of the basic ones are `SkiaDrawer`, `SkiaCarousel`,`SkiaGif` many more.
And `SkiaShell` is here to make possible the creation of totally drawn apps.

Accessibility is totally compatible and is on the roadmap.

Documentation is still very much in the ToDo 😔 state, while many examples are already here, more about them below.

### Quick Start

For install the nuget package __AppoMobi.Maui.DrawnUi__ , use latest *stable* version.
...
With DrawnUI the process is much easier than creating controls directly using low-level SkiaSharp primitives. We already have Lego-like bricks you to assemble from, compositing controls like we would with MAUI `Frame`, `Border`, etc.

Its obviously much easier to maintain and fix issues for a single drawn control than for all platform-dependent implementations.  

The process of creating a custom control is out of the scope of this article, could touch it later if this finds enough interest. It would be created like a usual MAUI control, the main difference being to use DrawnUI elements instead of MAUI controls.

Many examples are inside the Sandbox project as well as in demo apps.
...
We have recycled drawn cells with shadows scrolling smoothly while standart MAUI controls put more pressure on the device.
...
The replacement process is rather intuitive, you need a top container as `Canvas`, then basically you can replace standart MAUI controls with drawn, like replacing `Grid` with `SkiaLayout Type=”Grid”`, `VerticalStackLayout` with `SkiaLayout Type=”Column”`, `ScrollView` with `SkiaScroll` and so on..

This could very useful for existing MAUI libraries that propose composited custom controls, for example, a month calendar view with tons of native views/handlers for days, arrows, grids etc, would be replaced with just one `Canvas` native view, everthing else would become virtual.
...
Creating totally drawn NET MAUI apps remains in an experimental state as of today, and a lot of work remains to be done to make it a viable way to develop MAUI apps. At the same time i believe this is a very interesting challenge for all of us.

I would greatly appreciate your feedback, especially on what do you think of this approach and what spots in your apps would would be most useful to use drawn controls.
