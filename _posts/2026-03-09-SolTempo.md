...
Recent enhancements shipped with DrawnUI’s [SkiaCamera](https://github.com/taublast/DrawnUi/tree/main/src/Maui/Addons/DrawnUi.Maui.Camera) control (real-time video + audio processing) made creating this app possible. I will touch video processing with real-time encoding in the next article, meanwhile let’s have some fun with the incoming audio stream: this control can work in audio-only monitoring mode.
...
App is currently available in [AppStore](https://apps.apple.com/app/soltempo/id6759321593) and [GooglePlay](https://play.google.com/store/apps/details?id=com.appomobi.soltempo), you might consider installing it before further reading.
...
As the last step i quickly pluged [Plugin.Maui.AppRating](https://github.com/FabriBertani/Plugin.Maui.AppRating) library, to ask people to rate the app in the store: only once and only if app was used for more than 1 hour.  Was very easy to do, thanks to the plugin creator!
...
SolTempo is [fully open-source](https://github.com/taublast/SolTempo), Feel free to grab the code, experiment with the shader editor on Windows, and see what you can build.
